...
public:
    ...
    void record_no_lock(SubpathSample rec, Float statisticalWeight) {
        ...
        if (dr::isinf(rec.energy) || dr::isnan(rec.energy)) {
            return;
        }
    ...
    void build_distribution() {
        ...
        for (SubpathSample &subpath_sample : m_subpath_sample_list) {
            ...
            if (dr::isinf(subpath_sample.energy) || dr::isnan(subpath_sample.energy)) {
                continue;
            }
            ...
            if (global_sms_config.directional_struct == 0) { // ours
                ...
                direction_sample_for_type[subpath_sample.type].push_back(
                    dr::normalize(subpath_sample.x1 - subpath_sample.xD));
            } else if (global_sms_config.directional_struct == 1) { // dtree
                DTreeRecord rec = { dr::normalize(subpath_sample.x1 - subpath_sample.xD),
            ...
            for (const auto &[tau, direction_sample_for_type_tau] : direction_sample_for_type) {
                ...
                for (int i = 0; i < cached_radius.size(); i++) {
                    ...
                    for (const Vector3f &cache_dir : direction_sample_for_type_tau) {
                        if (dir == cache_dir)
                            continue;
                        float direction_dist = dr::dot(dir - cache_dir, dir - cache_dir);
    ...
    void init_product(ChainDistributionSamplingContext &ctx) {
        ...
        for (const auto &[tau, direction_sample_for_type_tau] : direction_sample_for_type) {
            ...
            for (int i = 0; i < direction_sample_for_type_tau.size(); i++) {
                ...
                float eps = 1e-6;
                if (dr::isinf(value) || dr::isnan(value) || value < eps)
                    value = eps;
    ...
    Vector3f noproduct_sample_omega(int tau, ref<Sampler> sampler, ChainDistributionSamplingContext &ctx) {
        ...
        if (cached_radius[guide_idx] < 0) {
            ...
            for (const Vector3f &cache_dir : direction_sample_for_type_tau) {
                if (guide_dir == cache_dir)
                    continue;
                float direction_dist = dr::dot(guide_dir - cache_dir, guide_dir - cache_dir);
                min_val              = std::min(min_val, direction_dist);
            }
            if (min_val < 1e9) {
                radius2 = min_val + eps;
        ...
        double W   = 1 + (std::log(xi[0] + std::exp(-2 * k) * (1.0 - xi[0]))) / k;
        double Wr  = std::sqrt(1.0 - W * W);
        Vector3f vmf_sample(Wr * std::sin(2 * math::Pi<double> * xi[1]),
                            Wr * std::cos(2 * math::Pi<double> * xi[1]), W);
    ...
    Vector3f product_sample_omega(int tau, ref<Sampler> sampler, ChainDistributionSamplingContext &ctx) {
        ...
        double k_brdf   = 2.0 / (alpha * alpha * 4 * (dr::abs(dr::dot(si.n, si.wi)) + 1e-6));
        double c_brdf   = 1 / (math::Pi<double> * alpha * alpha);
        Vector3f p_brdf = dr::normalize(2 * dr::dot(si.n, si.wi) * si.n - si.wi);
        ...
        Point2f xi = sampler->next_2d();
        double W   = 1 + (std::log(xi[0] + std::exp(-2 * k_radiance) * (1.0 - xi[0]))) / k_radiance;
        double Wr  = std::sqrt(1.0 - W * W);
        Vector3f vmf_sample(Wr * std::sin(2 * math::Pi<double> * xi[1]),
                            Wr * std::cos(2 * math::Pi<double> * xi[1]), W);
