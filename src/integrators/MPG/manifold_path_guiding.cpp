...
static thread_local std::vector<SubpathSample<Float, Spectrum>> recorded_samples_thread_local;
...
public:
    ...
    Mask sample_path(const SurfaceInteraction3f &si, const EmitterInteraction &ei, ref<Sampler> sampler) {
        ...
        Vector3f omega_seed     = dr::normalize(m_current_path[0].p - si.p);
        Mask success_solve      = manifold_walk.newton_solver(si, ei, m_current_path, m_offset_normals);
        Vector3f omega_solution = dr::normalize(m_current_path[0].p - si.p);
        if (!success_solve) {
            return false;
        }
        ...
        while (true) {
            ...
            if (bounce == sample_bounce) {
                ...
                if (success && success_e) {
                    ...
                    Vector3f direction             = dr::normalize(m_current_path[0].p - si.p);
                    ...
                    for (int _ = 0; _ < repeat_bernoulli; _++) {
                        ...
                        while (true) {
                            ...
                            if (success_trial) {
                                Vector3f direction_trial = dr::normalize(m_current_path[0].p - si.p);
                                int trial_tau            = getTau(m_current_path);

                                if (trial_tau == sampled_tau &&
                                    dr::abs(dr::dot(direction, direction_trial) - 1.f) < m_config.uniqueness_threshold) {
                                    break;
                                }
                            }
...
public:
    ...
    ManifoldPathGuidingIntegrator(const Properties &props) : Base(props) {
        ...
        // core
        m_sms_config.guided     = props.get<bool>("guided", false); // guide or not
        m_sms_config.train_auto = props.get<bool>("train_auto", false);

        // initialization
        m_sms_config.initial = props.get<int>("initial", 0);

        // ablation
        m_sms_config.ablation = props.get<int>("ablation", 0);

        // spatial struct
        m_sms_config.spatial_struct = props.get<int>("spatial_struct", 1);
        m_sms_config.spatial_filter = props.get<float>("spatial_filter", 0.1f);
        m_sms_config.knn_k          = props.get<int>("knn_k", -1);

        // directional struct
        m_sms_config.directional_struct = props.get<int>("directional_struct", 0);

        // online training related
        m_sms_config.budget_is_time = props.get<bool>("budget_is_time", true);
        m_sms_config.train_budget   = props.get<float>("train_budget", 0.3f);
        m_sms_config.train_fusion   = props.get<bool>("train_fusion", false);

        // utils
        m_sms_config.selective          = props.get<bool>("selective", false);
        m_sms_config.sms_strict_uniform = props.get<bool>("sms_strict_uniform", false);
        m_sms_config.sms_bounce_force   = props.get<int>("sms_bounce_force", 0);
        m_sms_config.sms_type_force     = props.get<int>("sms_type_force", 0);
        m_sms_config.seedid             = props.get<int>("seedid", 2023);
        m_sms_config.rep_ber            = props.get<int>("rep_ber", 1);

        // product sampling
        m_sms_config.product_sampling = props.get<bool>("product_sampling", false);

        // sms core
        m_sms_config.solver_threshold     = props.get<float>("solver_threshold", 1e-4f);
        m_sms_config.uniqueness_threshold = props.get<float>("uniqueness_threshold", 1e-4f);
        m_sms_config.max_trials           = props.get<int>("max_trials", 1000000);

        // sms legacy
        m_sms_config.halfvector_constraints = props.get<bool>("halfvector_constraints", false);
        m_sms_config.step_scale             = props.get<float>("step_scale", 1.f);
        m_sms_config.max_iterations         = props.get<int>("max_iterations", 20);

        // legacy
        m_sms_config.prob_uniform_train = props.get<float>("prob_uniform_train", 0.5f);
        m_sms_config.prob_uniform       = props.get<float>("prob_uniform", 0.5f);
        m_sms_config.sms_max_depth      = props.get<int>("sms_max_depth", 99);
