...
public:
    ...
    Float computeOverlappingArea(const Point2 &min1, const Point2 &max1, const Point2 &min2, const Point2 &max2) {
        Float lengths[2];
        for (int i = 0; i < 2; ++i) {
            lengths[i] = dr::max(dr::min(max1[i], max2[i]) - dr::max(min1[i], min2[i]), 0.0f);
        }
...
public:
    ...
    Float mean() const {
        ...
        const Float factor = 1 / (math::Pi<Float> * 4 * m_atomic.statisticalWeight);
    ...
    Float pdf(Point2 p) const {
        if (!(mean() > 0)) {
            return 1 / (4 * math::Pi<Float>);
        }

        return m_nodes[0].pdf(p, m_nodes) / (4 * math::Pi<Float>);
    }
    ...
    void reset(const DTree &previousDTree, int newMaxDepth, Float subdivisionThreshold) {
        ...
        while (!nodeIndices.empty()) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                const Float fraction          = total > 0 ? (otherNode.sum(i) / total) : dr::pow(0.25f, sNode.depth);
                ...
                if (sNode.depth < newMaxDepth && fraction > subdivisionThreshold) {
                    ...
                    if (m_nodes.size() > std::numeric_limits<uint16_t>::dr::max()) {
...
public:
    ...
    static Vector3 canonicalToDir(Point2 p) {
        ...
        const Float phi      = 2 * math::Pi<Float> * p[1];

        const Float sinTheta = dr::sqrt(1 - cosTheta * cosTheta);
        Float sinPhi, cosPhi;
        sinPhi = dr::sin(phi);
        cosPhi = dr::cos(phi);
    ...
    static Point2 dirToCanonical(const Vector3 &d) {
        if (!dr::isfinite(d[0]) || !dr::isfinite(d[1]) || !dr::isfinite(d[2])) {
            return { 0, 0 };
        }

        const Float cosTheta = dr::min(dr::max(d[2], -1.0f), 1.0f);
        Float phi            = std::atan2(d[1], d[0]);
        while (phi < 0)
            phi += 2.0 * math::Pi<Float>;

        return { (cosTheta + 1) / 2, phi / (2 * math::Pi<Float>) };
    }
