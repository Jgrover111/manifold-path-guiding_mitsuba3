...
MI_INLINE auto harmonic_mean(const Value &value) {
    return dr::rcp(dr::mean(dr::rcp(value)));
}
...
template <typename Float_, typename Spectrum_> struct SpecularManifold {
    ...
    static EmitterInteraction sample_emitter_interaction(const SurfaceInteraction3f &si,
        ...
        UInt32 index = dr::min(UInt32(emitter_sample * (ScalarFloat) emitters.size()), (uint32_t) emitters.size() - 1);
    ...
    static std::pair<Mask, ManifoldVertex> emitter_interaction_to_vertex(const Scene *scene,
        ...
        if (ei.is_area()) {
            // Area emitters
            Vector3f d_tmp = dr::normalize(ei.p - p);
            ...
            Vector3f d         = dr::normalize(ei.p - p);
            ...
            // Point emitters
            ManifoldVertex vy(ei.p);
            Vector3f d = dr::normalize(p - ei.p);
            vy.n = vy.gn = d;
    ...
    static EmitterInteraction emitter_interaction(const Scene *scene, const SurfaceInteraction3f &si,
        ...
        if (shape) {
            ei.p = si_emitter.p;
            ei.n = si_emitter.n;
            ei.d = dr::normalize(si_emitter.p - si.p);
    ...
    static MI_INLINE Point2f sample_gaussian(const Point2f &mu, const Matrix2f &sigma, const Point2f &sample) {
        ...
        Float sigma_x = dr::sqrt(sigma(0, 0)), sigma_y = dr::sqrt(sigma(1, 1));

        Float rho = sigma(1, 0) / (sigma_x * sigma_y);
        Matrix2f P(0.5f, 0.5f, 0.5f, 0.5f);
        Matrix2f Q(0.5f, -0.5f, -0.5f, 0.5f);
        Matrix2f A = dr::sqrt(1.f + rho) * P + dr::sqrt(1.f - rho) * Q;
        p          = A * p;
    ...
    static MI_INLINE std::pair<Mask, Vector3f> reflect(const Vector3f &w, const Normal3f &n) {
        return std::make_pair(true, 2.f * dr::dot(w, n) * n - w);
    }
    ...
    static MI_INLINE std::pair<Vector3f, Vector3f> d_reflect(const Vector3f &w, const Vector3f &dw_du,
        ...
        Float dot_w_n = dr::dot(w, n), dot_dwdu_n = dr::dot(dw_du, n), dot_dwdv_n = dr::dot(dw_dv, n), dot_w_dndu = dr::dot(w, dn_du),
    ...
    static MI_INLINE std::pair<Vector3f, Vector3f> d_refract(const Vector3f &w, const Vector3f &dw_du,
        ...
        Float dot_w_n = dr::dot(w, n), dot_dwdu_n = dr::dot(dw_du, n), dot_dwdv_n = dr::dot(dw_dv, n), dot_w_dndu = dr::dot(w, dn_du),
              dot_w_dndv = dr::dot(w, dn_dv);
        Float root       = dr::sqrt(1.f - eta * eta * (1.f - dot_w_n * dot_w_n));
    ...
    static MI_INLINE std::tuple<Float, Float, Float, Float> d_sphcoords(const Vector3f &w, const Vector3f &dw_du,
        ...
        Float d_acos     = -dr::rcp(dr::safe_sqrt(1.f - w[2] * w[2]));
    ...
    static Spectrum specular_reflectance(const Scene *scene, const SurfaceInteraction3f &si_,
        ...
        Vector3f wo = dr::normalize(x1 - x0);
        ...
        for (size_t k = 0; k < v.size(); ++k) {
            ...
            if (k < v.size() - 1) {
                ...
                wo                        = dr::normalize(distribution_next - si.p);
            } else {
                // Connect with light source
                if (ei.is_directional()) {
                    wo = ei.d;
                } else {
                    wo = dr::normalize(ei.p - si.p);
                }
            }
            ...
            if (bsdf->roughness() > 0.f) {
                ...
                are omitted: 1) dr::abs_dot(wo, n) is part of BSDF::eval 2)
                dr::abs_dot(h, n)  is part of the Microfacet distr. (also in
                BSDF::eval) */
                Vector3f h_l;
                if (reflection) {
                    h_l = dr::normalize(si.wi + wo_l);
                    f *= 4.f * dr::abs_dot(wo_l, h_l);
                } else {
                    Float eta = harmonic_mean(dr::real(ior));
                    if (Frame3f::cos_theta(si.wi) < 0.f) {
                        eta = dr::rcp(eta);
                    }
                    h_l = -dr::normalize(si.wi + eta * wo_l);
                    f *= dr::square(dr::dot(si.wi, h_l) + eta * dr::dot(wo_l, h_l)) / (eta * eta * dr::abs_dot(wo_l, h_l));
                }
                ...
                if (reflection) {
                    if (dr::all(dr::eq(dr::imag(ior), 0.f))) {
                        ...
                        f = fresnel_conductor(Spectrum(dr::abs(cos_theta)), ior);
                    }
                } else {
                    Float eta = harmonic_mean(dr::real(ior));
                    if (cos_theta < 0.f) {
        ...
        // there
        if (!ei.is_directional()) {
            ray = Ray3f(si.p, dr::normalize(ei.p - si.p), math::RayEpsilon<Float> * (1.f + dr::hmax(dr::abs(si.p))),
                        dr::norm(ei.p - si.p) * (1.f - math::RayEpsilon<Float>), si.time, si.wavelengths);
        }
    ...
    static Float geometric_term(const ManifoldVertex &vx, const ManifoldVertex &vy,
        ...
        for (size_t i = 0; i < k - 1; ++i) {
            ...
            if (v[i].fixed_direction) {
                ...
                Vector3f dc_du_cur = -ilo * (v[i].dp_du - wo * dr::dot(wo, v[i].dp_du)),
                         dc_dv_cur = -ilo * (v[i].dp_dv - wo * dr::dot(wo, v[i].dp_dv));
                v[i].dC_dx_cur     = Matrix2f(dr::dot(dc_du_cur, v[i].dp_du), dr::dot(dc_dv_cur, v[i].dp_du),
                                              dr::dot(dc_du_cur, v[i].dp_dv), dr::dot(dc_dv_cur, v[i].dp_dv));

                // Derivative of directional constraint w.r.t. x_{i+1}
                Vector3f dc_du_next = ilo * (v[i + 1].dp_du - wo * dr::dot(wo, v[i + 1].dp_du)),
                         dc_dv_next = ilo * (v[i + 1].dp_dv - wo * dr::dot(wo, v[i + 1].dp_dv));
                v[i].dC_dx_next     = Matrix2f(dr::dot(dc_du_next, v[i].dp_du), dr::dot(dc_dv_next, v[i].dp_du),
                                               dr::dot(dc_du_next, v[i].dp_dv), dr::dot(dc_dv_next, v[i].dp_dv));
                continue;
            }
            ...
            if (dr::dot(wi, v[i].gn) < 0.f) {
                eta = dr::rcp(eta);
            }
            Vector3f h = wi + eta * wo;
            if (eta != 1.f)
                h *= -1.f;
            Float ilh = dr::rcp(dr::norm(h));
            h *= ilh;
            ...
            Float dot_dpdu_n = dr::dot(v[i].dp_du, v[i].n), dot_dpdv_n = dr::dot(v[i].dp_dv, v[i].n);
            ...
            if (i > 0) {
                dh_du = ili * (v[i - 1].dp_du - wi * dr::dot(wi, v[i - 1].dp_du));
                dh_dv = ili * (v[i - 1].dp_dv - wi * dr::dot(wi, v[i - 1].dp_dv));
                dh_du -= h * dr::dot(dh_du, h);
                dh_dv -= h * dr::dot(dh_dv, h);
                if (eta != 1.f) {
                    dh_du *= -1.f;
                    dh_dv *= -1.f;
                }

                v[i].dC_dx_prev = Matrix2f(dr::dot(s, dh_du), dr::dot(s, dh_dv), dr::dot(t, dh_du), dr::dot(t, dh_dv));
            }

            // Derivative of specular constraint w.r.t. x_{i}
            dh_du = -v[i].dp_du * (ili + ilo) + wi * (dr::dot(wi, v[i].dp_du) * ili) + wo * (dr::dot(wo, v[i].dp_du) * ilo);
            dh_dv = -v[i].dp_dv * (ili + ilo) + wi * (dr::dot(wi, v[i].dp_dv) * ili) + wo * (dr::dot(wo, v[i].dp_dv) * ilo);
            dh_du -= h * dr::dot(dh_du, h);
            dh_dv -= h * dr::dot(dh_dv, h);
            if (eta != 1.f) {
                dh_du *= -1.f;
            ...
            Float dot_h_n = dr::dot(h, v[i].n), dot_h_dndu = dr::dot(h, v[i].dn_du), dot_h_dndv = dr::dot(h, v[i].dn_dv);

            v[i].dC_dx_cur = Matrix2f(dr::dot(dh_du, s) - dr::dot(v[i].dp_du, v[i].dn_du) * dot_h_n - dot_dpdu_n * dot_h_dndu,
                                      dr::dot(dh_dv, s) - dr::dot(v[i].dp_du, v[i].dn_dv) * dot_h_n - dot_dpdu_n * dot_h_dndv,
                                      dr::dot(dh_du, t) - dr::dot(v[i].dp_dv, v[i].dn_du) * dot_h_n - dot_dpdv_n * dot_h_dndu,
                                      dr::dot(dh_dv, t) - dr::dot(v[i].dp_dv, v[i].dn_dv) * dot_h_n - dot_dpdv_n * dot_h_dndv);

            // Derivative of specular constraint w.r.t. x_{i+1}
            dh_du = ilo * (v[i + 1].dp_du - wo * dr::dot(wo, v[i + 1].dp_du));
            dh_dv = ilo * (v[i + 1].dp_dv - wo * dr::dot(wo, v[i + 1].dp_dv));
            dh_du -= h * dr::dot(dh_du, h);
            dh_dv -= h * dr::dot(dh_dv, h);
            if (eta != 1.f) {
                dh_du *= -1.f;
            ...
            v[i].dC_dx_next = Matrix2f(dr::dot(s, dh_du), dr::dot(s, dh_dv), dr::dot(t, dh_du), dr::dot(t, dh_dv));
        }
        ...
        if (vy.fixed_direction) {
            ...
            Vector3f d = dr::normalize(v[k - 1].p - v[k - 2].p);
            G /= dr::abs_dot(d, v[k - 1].n);
            return G;
        } else {
            ...
            dx1_dxend    = dr::min(dx1_dxend, Float(10.f));
            Vector3f d   = vx.p - v[0].p;
            Float inv_r2 = dr::rcp(dr::squared_norm(d));
            d *= dr::sqrt(inv_r2);
            Float dw0_dx1 = dr::abs_dot(d, v[0].gn) * inv_r2;
    ...
    static Float invert_tridiagonal_geo(std::vector<ManifoldVertex> &v) {
        ...
        return dr::abs(det(-v[0].inv_lambda));
    }
    ...
    Mask newton_solver(const SurfaceInteraction3f &si, const EmitterInteraction &ei,
        ...
        while (iterations < m_config.max_iterations) {
            ...
            for (size_t i = 0; i < current_path.size(); ++i) {
                ...
                if (dr::norm(v.C) > m_config.solver_threshold) {
                    converged = false;
                    break;
                }
            }
            ...
            if (!project_success) {
                ...
                beta              = dr::min(Float(1), Float(2) * beta);
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            Vector3f wi                           = dr::normalize(x_prev - x_cur);
            Vector3f wo                           = at_endpoint_with_fixed_direction ? ei.d : dr::normalize(x_next - x_cur);

            Float cos_theta_i = dr::dot(current_path[i].gn, wi), cos_theta_o = dr::dot(current_path[i].gn, wo);
    ...
    Mask compute_step_halfvector(const SurfaceInteraction3f &si, const EmitterInteraction &ei,
        ...
        for (size_t i = 0; i < k; ++i) {
            ...
            if (i > 0) {
                ...
                dh_du = ili * (si.dp_du - wi * dr::dot(wi, si.dp_du));
                dh_dv = ili * (si.dp_dv - wi * dr::dot(wi, si.dp_dv));
            }
            ...
            v[i].dC_dx_prev = Matrix2f(dr::dot(v[i].s, dh_du), dr::dot(v[i].s, dh_dv), dr::dot(v[i].t, dh_du), dr::dot(v[i].t, dh_dv));
            //}
            ...
            if (at_endpoint_with_fixed_direction) {
                ...
                // simplifies.
                dh_du = ili * (-v[i].dp_du + wi * dr::dot(wi, v[i].dp_du));
                dh_dv = ili * (-v[i].dp_dv + wi * dr::dot(wi, v[i].dp_dv));
            } else {
                // Standard case for fixed emitter position
                dh_du = -v[i].dp_du * (ili + ilo) + wi * (dr::dot(wi, v[i].dp_du) * ili) + wo * (dr::dot(wo, v[i].dp_du) * ilo);
                dh_dv = -v[i].dp_dv * (ili + ilo) + wi * (dr::dot(wi, v[i].dp_dv) * ili) + wo * (dr::dot(wo, v[i].dp_dv) * ilo);
            ...
            v[i].dC_dx_cur = Matrix2f(dr::dot(v[i].ds_du, h) + dr::dot(v[i].s, dh_du), dr::dot(v[i].ds_dv, h) + dr::dot(v[i].s, dh_dv),
                                      dr::dot(v[i].dt_du, h) + dr::dot(v[i].t, dh_du), dr::dot(v[i].dt_dv, h) + dr::dot(v[i].t, dh_dv));
            ...
            if (i < k - 1) {
                ...
                if (success_e) {
                    dh_du = ilo * (vy.dp_du - wo * dr::dot(wo, vy.dp_du));
                    dh_dv = ilo * (vy.dp_dv - wo * dr::dot(wo, vy.dp_dv));
                }
            }
            dh_du -= h * dr::dot(dh_du, h);
            dh_dv -= h * dr::dot(dh_dv, h);
            ...
            v[i].dC_dx_next = Matrix2f(dr::dot(v[i].s, dh_du), dr::dot(v[i].s, dh_dv), dr::dot(v[i].t, dh_du), dr::dot(v[i].t, dh_dv));
            //}

            // Evaluate specular constraint
            Vector2f H(dr::dot(v[i].s, h), dr::dot(v[i].t, h));
    ...
    Mask compute_step_anglediff(const SurfaceInteraction3f &si, const EmitterInteraction &ei,
        ...
        for (size_t i = 0; i < k; ++i) {
            ...
            if (at_endpoint_with_fixed_direction) {
                ...
                // Standard case for fixed emitter position
                dwo_du_cur = -ilo * (v[i].dp_du - wo * dr::dot(wo, v[i].dp_du));
                dwo_dv_cur = -ilo * (v[i].dp_dv - wo * dr::dot(wo, v[i].dp_dv));
            }
            ...
            Vector3f dwi_du_cur = -ili * (v[i].dp_du - wi * dr::dot(wi, v[i].dp_du)),
                     dwi_dv_cur = -ili * (v[i].dp_dv - wi * dr::dot(wi, v[i].dp_dv));
            ...
            if (valid_i_refr_i) {
                ...
                if (i > 0) {
                    dwi_du_prev = ili * (v[i - 1].dp_du - wi * dr::dot(wi, v[i - 1].dp_du)),
                    dwi_dv_prev = ili * (v[i - 1].dp_dv - wi * dr::dot(wi, v[i - 1].dp_dv));
                } else {
                    dwi_du_prev = ili * (si.dp_du - wi * dr::dot(wi, si.dp_du)),
                    dwi_dv_prev = ili * (si.dp_dv - wi * dr::dot(wi, si.dp_dv));
                }
                // Vector3f dwo_du_prev = ilo * (v[i-1].dp_du - wo*dr::dot(wo,
                // v[i-1].dp_du)),  // = 0
                //          dwo_dv_prev = ilo * (v[i-1].dp_dv - wo*dr::dot(wo,
                //          v[i-1].dp_dv));  // = 0
                ...
                if (i < k - 1) {
                    // Vector3f dwi_du_next = ili * (v[i+1].dp_du - wi*dr::dot(wi,
                    // v[i+1].dp_du)),  // = 0 dwi_dv_next = ili * (v[i+1].dp_dv
                    // - wi*dr::dot(wi, v[i+1].dp_dv));  // = 0
                    dwo_du_next = ilo * (v[i + 1].dp_du - wo * dr::dot(wo, v[i + 1].dp_du)),
                    dwo_dv_next = ilo * (v[i + 1].dp_dv - wo * dr::dot(wo, v[i + 1].dp_dv));
                } else {
                    auto [success_e, vy] = SpecularManifold::emitter_interaction_to_vertex(m_scene, ei, v[i].p, si.time,
                                                                                           si.wavelengths, false);
                    if (success_e) {
                        dwo_du_next = ilo * (vy.dp_du - wo * dr::dot(wo, vy.dp_du)),
                        dwo_dv_next = ilo * (vy.dp_dv - wo * dr::dot(wo, vy.dp_dv));
                    }
                }
            ...
            if (valid_o_refr_o && !success_i) {
                ...
                // wo*dr::dot(wo, v[i-1].dp_dv));  // = 0 auto [dwoi_du_prev,
